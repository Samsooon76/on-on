...
export function NumberPurchase(props: {
  ...
  return <dialog ref={dialog} className="number-dialog" aria-labelledby="number-dialog-title" onCancel={(event) => { event.preventDefault(); if (!busy) props.onClose(); }}>
    ...
    <p className="number-intro">Un numéro local pour les appels uniquement, attribué automatiquement à <strong>{props.email}</strong>.</p>
    {error && <div className="number-feedback number-error" role="alert">{error}</div>}
    {order?.status === "completed" ? <div className="number-result" role="status"><span className="number-success" aria-hidden="true">✓</span><h3>Votre numéro est prêt</h3><p className="number-result-phone">{order.phoneNumber}</p><p>Votre ligne locale est prête pour les appels entrants et sortants.</p><button className="button button-primary" disabled={busy} onClick={() => void startCalling()}>{busy ? "Ouverture…" : "Passer un appel"}</button></div>
      ...
      : selected ? <div className="number-confirmation"><button className="number-back" disabled={busy} onClick={() => setSelected(null)}>← Changer de numéro</button><h3>Confirmer votre numéro</h3><p className="number-result-phone">{selected.phoneNumber}</p><div className="number-price"><strong>{priceLabel(selected)}</strong><span> / mois HT · {selected.currency}</span></div><p>Abonnement facturé sur le compte Twilio de votre organisation dès la commande, puis chaque mois. Numéro local, appels entrants et sortants uniquement. Frais d’appels et taxes en supplément.</p>{expired && <p className="number-feedback" role="status">Cette sélection a expiré. Relancez la recherche.</p>}<button className="button button-primary button-wide" disabled={busy || expired} onClick={() => void purchase()}>{busy ? "Commande en cours…" : `Commander · ${priceLabel(selected)} / mois`}</button>{expired && <button className="number-back" onClick={() => void search()}>Actualiser les numéros</button>}</div>
      : <><form className="number-search" onSubmit={(event) => { event.preventDefault(); void search(); }}><label>Pays du numéro<select value={country} disabled={busy} onChange={(event) => { setCountry(event.target.value); setOffers(null); setError(""); }}>{countries.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}</select></label><button className="button button-primary" disabled={busy}>{busy ? "Recherche…" : "Voir les numéros"}</button></form><p className="number-note">Numéros locaux · Appels uniquement. Le prix est confirmé avant l’achat.</p>{offers?.length === 0 && <div className="number-feedback" role="status">Aucun numéro local compatible avec les appels n’est disponible dans ce pays pour le moment. Réessayez plus tard ou choisissez un autre pays.</div>}{offers && offers.length > 0 && <ul className="number-offers" aria-label="Numéros disponibles">{offers.map((offer) => <li key={offer.quoteId}><div><strong>{offer.phoneNumber}</strong><small>Local · Appels uniquement</small></div><div className="number-offer-price"><strong>{priceLabel(offer)}</strong><small>/ mois HT · {offer.currency}</small></div><button className="button" onClick={() => setSelected(offer)}>Choisir</button></li>)}</ul>}</>}
  </dialog>;
}
