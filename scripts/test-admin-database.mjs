...
try {
  ...
  const results = [];
  for (const file of ['admin_and_ivr.test.sql', 'live_call_center.test.sql', 'mcp_integrations.test.sql', 'customer_webhooks.test.sql']) results.push(...await db.exec(await readFile(new URL('../supabase/tests/' + file, import.meta.url), 'utf8')));
